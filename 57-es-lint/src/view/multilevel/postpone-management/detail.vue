    ...
    export default {
        ...
        methods: {
            ...
            openOAUploadModal (item) {
                ajax.listPostponeOaPictureByApply({
                    archiveId: this.archiveId,
            ...
            getFileDetail (file) {
                ajax.getPostponeFileDetail({archiveDocumentId: file.archiveDocumentId}).then(res => {
                    let {error_code, message, data} = res.data;
                    if (error_code) {
                        this.$Message.error(message);
                    } else {
